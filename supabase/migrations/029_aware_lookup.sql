-- =====================================================================
-- MEDD-App — 029 Tabla de referencia AWaRe OMS (Sección 11, Cambio 1b · R9)
--
-- Fuente OFICIAL (verbatim, NO generada de memoria): WHO Access, Watch, Reserve
-- (AWaRe) classification of antibiotics 2025 — publicación WHO B09489. Cargada como
-- tabla de referencia versionada. Marca la categoría Access/Watch/Reserve por
-- antibiótico (INN) con su código ATC nivel 5. Combinaciones «no recomendadas» en
-- tabla aparte. La categoría AWaRe se DERIVA por join (nunca se digita en captura).
--
-- Nota de versión: el prompt citaba AWaRe 2023 (WHO-MHP-HPS-EML-2023.04); el
-- stakeholder aportó la 2025 (B09489), más reciente — se usa esa y se declara aquí.
--
-- Las formulaciones TÓPICAS/OFTÁLMICAS quedan fuera del alcance sistémico de AWaRe;
-- esa distinción es por FORMULACIÓN (vía), no por sustancia, y se aplica en el
-- análisis (no vive en esta tabla, que es por principio activo).
--
-- Idempotente: create table if not exists + on conflict do nothing. RLS de solo
-- lectura para autenticados (dato de referencia). Rollback: drop table.
-- =====================================================================

create table if not exists public.aware_lookup (
  antibiotic  text primary key,          -- INN (inglés), como publica la OMS
  class       text,
  atc_code    text,                       -- ATC nivel 5 (llave de derivación)
  category    text not null check (category in ('Access','Watch','Reserve')),
  eml         text,
  version     text not null default 'AWaRe 2025 (WHO B09489)'
);
create index if not exists idx_aware_lookup_atc on public.aware_lookup(atc_code);

alter table public.aware_lookup enable row level security;
do $$ begin
  if not exists (select 1 from pg_policies where tablename='aware_lookup' and policyname='aware_lookup_read') then
    create policy aware_lookup_read on public.aware_lookup for select to authenticated using (true);
  end if;
end $$;

insert into public.aware_lookup (antibiotic,class,atc_code,category,eml) values
  ...
  ('Amoxicillin/sulbactam','Beta-lactam/beta-lactamase-inhibitor','J01CR02','Access','No'),
  ...
  ('Azanidazole','Imidazoles','P01AB04','Access','No'),
  ...
  ('Aztreonam/avibactam','Monobactam/beta-lactamase-inhibitor','J01DF51','Reserve','No'),
  ...
  ('Benzathine-phenoxymethylpenicillin','Penicillins','J01CE10','Access','No'),
  ...
  ('Cefoselis','Fourth-generation-cephalosporins','not assigned','Watch','No'),
  ...
  ('Gepotidacin','Triazaacenaphthylenes','J01XX13','Watch','No'),
  ...
  ('Micronomicin','Aminoglycosides','not assigned','Watch','No'),
  ...
  ('Nalidixic-acid','Quinolones','J01MB02','Watch','No'),
  ...
  ('Nimorazole','Imidazoles','P01AB06','Access','No'),
  ...
  ('Paromomycin_oral','Aminoglycosides','A07AA06','Watch','No'),
  ...
  ('Propenidazole','Imidazoles','P01AB05','Access','No'),
  ...
  ('Satranidazole','Imidazoles','P01AB08','Access','No'),
  ...
  ('Ticarcillin/clavulanic-acid','Beta-lactam/beta-lactamase-inhibitor','J01CR03','Watch','No'),
...
on conflict (antibiotic) do nothing;

create table if not exists public.aware_not_recommended (
  antibiotic text primary key
);
alter table public.aware_not_recommended enable row level security;
do $$ begin
  if not exists (select 1 from pg_policies where tablename='aware_not_recommended' and policyname='aware_nr_read') then
    create policy aware_nr_read on public.aware_not_recommended for select to authenticated using (true);
  end if;
end $$;
insert into public.aware_not_recommended (antibiotic) values
  ('acetylspiramycin/metronidazole'),
  ('amikacin/cefepime'),
  ('amoxicillin/bacillus coagulans/cloxacillin'),
  ('amoxicillin/bacillus coagulans/dicloxacillin'),
  ('amoxicillin/clavulanic acid/lactic ferments'),
  ('amoxicillin/clavulanic acid/lactobacillus acidophilus'),
  ('amoxicillin/clavulanic acid/nimesulide'),
  ('amoxicillin/cloxacillin'),
  ('amoxicillin/cloxacillin/lactic acid'),
  ('amoxicillin/cloxacillin/lactobacillus acidophilus/serrapeptase'),
  ('amoxicillin/cloxacillin/lactobacillus lactis'),
  ('amoxicillin/cloxacillin/serrapeptase'),
  ('amoxicillin/dicloxacillin'),
  ('amoxicillin/dicloxacillin/saccharomyces boulardii'),
  ('amoxicillin/flucloxacillin'),
  ('amoxicillin/flucloxacillin/lactobacillus acidophilus'),
  ('amoxicillin/metronidazole'),
  ('amoxicillin/pivsulbactam'),
  ('ampicillin/bacillus coagulans/cloxacillin'),
  ('ampicillin/cloxacillin'),
  ('ampicillin/cloxacillin/lactobacillus acidophilus'),
  ('ampicillin/cloxacillin/saccharomyces boulardii'),
  ('ampicillin/dicloxacillin'),
  ('ampicillin/dicloxacillin/lactobacillus acidophilus'),
  ('ampicillin/flucloxacillin'),
  ('ampicillin/lidocaine/sulbactam'),
  ('ampicillin/oxacillin'),
  ('ampicillin/sultamicillin'),
  ('ascorbic acid/metamizole sodium/penicillin g /streptomycin'),
  ('azithromycin/cefixime'),
  ('azithromycin/cefixime/lactobacillus acidophilus'),
  ('azithromycin/cefpodoxime proxetil'),
  ('azithromycin/fluconazole/secnidazole'),
  ('azithromycin/levofloxacin'),
  ('azithromycin/ofloxacin'),
  ('benzyl penicillin/streptomycin'),
  ('benzylpenicillin/procaine-benzylpenicillin/benzathine-benzylpenicillin'),
  ('bromelains/doxycycline/lactobacillus reuteri/lactobacillus rhamnosus/ornidazole'),
  ('bromhexine/sulfamethoxazole/trimethoprim'),
  ('cefaclor/clavulanic acid'),
  ('cefadroxil/clavulanic acid'),
  ('cefadroxil/trimethoprim'),
  ...
  ('cefixime/clavulanic acid/lactobacillus acidophilus'),
  ('cefixime/cloxacillin'),
  ('cefixime/cloxacillin/lactobacillus acidophilus'),
  ('cefixime/dicloxacillin'),
  ('cefixime/lactobacillus acidophilus/ofloxacin'),
...
comment on table public.aware_lookup is 'Clasificación AWaRe OMS 2025 (WHO B09489), verbatim de la fuente oficial. Access/Watch/Reserve por antibiótico + ATC5. Derivación de aware_categoria por join, nunca capturada.';
