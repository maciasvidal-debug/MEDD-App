-- =====================================================================
-- MEDD-App — 022 Catálogo DANE de municipios en la base + FK (RBQM R4)
--
-- Contexto (piloto): `ciudad` traía 39 grafías para pocos municipios y
-- `departamento` 100% vacío. El cliente ya canonicaliza contra el catálogo
-- DIVIPOLA bundleado (src/lib/divipola.data.ts, 1.122 municipios, fuente oficial
-- datos.gov.co, verificado 1:1 en divipola.test.ts) y deriva el departamento por
-- cascada — el control en captura ya se cumple. Lo que faltaba (anexo R4) es
-- llevar el catálogo a la BASE como dato de referencia y dar integridad
-- referencial a la geografía capturada.
--
-- Qué añade:
--   1) Tabla `municipio_dane(codigo PK, nombre, departamento)` sembrada desde la
--      MISMA fuente oficial del bundle (no se inventan códigos). RLS de solo
--      lectura para autenticados (dato de referencia, no sensible).
--   2) Columna `surveys.municipio_codigo` con FK a municipio_dane(codigo),
--      NULLABLE. La app la deriva del par canónico (ciudad, departamento) al
--      sincronizar (sync-mapping.toRow → lookupCodigo). Históricos = NULL (pasan
--      la FK) y se pueden back-fillear por (ciudad, departamento) si se decide.
--   3) CHECK de la regla «Otro» en los text[] de motivos/disposición: el texto
--      libre `*_otro` solo puede tener contenido si «Otro» está seleccionado.
--      NOT VALID (grandfathering de cualquier inconsistencia del piloto).
--
-- Idempotente: create table if not exists; insert ... on conflict do nothing;
-- add column if not exists; guardas pg_constraint. Seguro de re-ejecutar.
--
-- Rollback:
--   alter table public.surveys drop constraint if exists chk_disp_final_otro;
--   alter table public.surveys drop constraint if exists chk_mot_no_consumo_otro;
--   alter table public.surveys drop constraint if exists chk_mot_venc_otro;
--   alter table public.surveys drop constraint if exists surveys_municipio_codigo_fkey;
--   alter table public.surveys drop column if exists municipio_codigo;
--   drop table if exists public.municipio_dane;
-- =====================================================================

create table if not exists public.municipio_dane (
  codigo       text primary key,
  nombre       text not null,
  departamento text not null
);

alter table public.municipio_dane enable row level security;

do $$ begin
  if not exists (select 1 from pg_policies where tablename = 'municipio_dane' and policyname = 'municipio_dane_read') then
    create policy municipio_dane_read on public.municipio_dane
      for select to authenticated using (true);
  end if;
end $$;

-- Seed oficial DIVIPOLA (1.122 municipios). Idempotente por on conflict.
insert into public.municipio_dane (codigo, nombre, departamento) values
...
-- FK de la respuesta al catálogo (nullable; históricos NULL pasan la FK).
alter table public.surveys
  add column if not exists municipio_codigo text;

do $$ begin
  if not exists (select 1 from pg_constraint where conname = 'surveys_municipio_codigo_fkey') then
    alter table public.surveys
      add constraint surveys_municipio_codigo_fkey
      foreign key (municipio_codigo) references public.municipio_dane(codigo)
...
comment on table public.municipio_dane is
  'Catálogo oficial DANE/DIVIPOLA (1.122 municipios). Dato de referencia sembrado desde datos.gov.co. FK destino de surveys.municipio_codigo.';
comment on column public.surveys.municipio_codigo is
  'Código DANE del municipio (FK municipio_dane). Derivado del par canónico (ciudad, departamento) al sincronizar. NULL en históricos.';
